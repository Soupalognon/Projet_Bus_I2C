...
{   
    ...
    unsigned int ID = 0x01;    //ID sur 7 bits
    unsigned int data = 0x2A;       //Config de la donnée à envoyer
    //TIMER_I2C = ACTIVE;       //Active le timer réception des données toutes les 200ms
    
    while(1)
    {
        envoiInformationI2C(ID, data);
        delay_ms(1000);
